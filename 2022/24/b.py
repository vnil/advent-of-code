...
while deq:
    ...
    if ((x, y), steps, trips) in visited:
        continue
    visited.add(((x, y), steps, trips))
    ...
    for dx, dy in deltas:
        ...
        if 0 <= tx < len(content[0]) and 0 <= ty < len(content):
            deq.append(((tx, ty), steps+1, trips))
        deq.append(((x, y), steps+1, trips))