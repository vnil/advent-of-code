...
for row in content:
    ...
    def explore(minutes_left, robots, wallet):
        ...
        ore_spent = geode_robot_price_in_ore * o_geode

        o_obsidian = min((wallet[0] - ore_spent) // obsidian_robot_price_in_ore, wallet[1] // obsidian_robot_price_in_clay)
        ore_spent += obsidian_robot_price_in_ore * o_obsidian

        o_clay = (wallet[0] - ore_spent) // clay_robot_price_in_ore
        ore_spent += clay_robot_price_in_ore * o_clay
        o_ore = (wallet[0] - ore_spent) // ore_robot_price_in_ore
        ...
        for i in range(4):
            if spent[i] > wallet[i]:
                print(i, spent, wallet, minutes_left)
                ok = False
                break
        ...
        for i in range(4):    
            #end of the minute
            new_wallet[i] -= spent[i]
        

        explored.append(explore(minutes_left - 1, new_robots, tuple(new_wallet)))
